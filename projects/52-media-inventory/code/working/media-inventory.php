 ...
 * Version: 1.0
 * Provides detailed inventory of all media files with sizes and counts
 */
...
function media_inventory_ajax_scan()
{
    ...
    $batch_size = 10; // Reduced to 10 attachments at a time for better stability
    ...
    foreach ($attachments as $attachment_id) {
        try {
            ...
            if (!$file_path || !$mime_type) {
                $errors[] = "Skipped attachment ID $attachment_id: missing file path or MIME type";
                continue;
            }

            $category = media_inventory_get_category($mime_type);
            ...
            if (file_exists($file_path)) {
                ...
                $file_info = [
                    'path' => str_replace($upload_basedir, '', $file_path),
                ...
                // Get image dimensions for this specific file
                if (strpos($mime_type, 'image/') === 0) {
                    $image_info = @getimagesize($file_path);
                    if ($image_info) {
                        $file_info['dimensions'] = $image_info[0] . ' × ' . $image_info[1] . 'px';
                        $item_data['dimensions'] = $file_info['dimensions']; // Keep main dimensions for overall item
                    }
                }
            ...
            // For fonts, extract font family
            if ($category === 'Fonts') {
                $item_data['font_family'] = media_inventory_get_font_family($title, $file_path);
            }

            // For images, get thumbnail and check additional sizes
            if (strpos($mime_type, 'image/') === 0) {
                ...
                if ($metadata && isset($metadata['sizes'])) {
                    ...
                    foreach ($metadata['sizes'] as $size_name => $size_data) {
                        ...
                        if (file_exists($size_file) && !isset($processed_files[$size_file_key])) {
                            ...
                            $file_info = [
                                'path' => str_replace($upload_basedir, '', $size_file),
            ...
            $errors[] = "Error processing attachment ID $attachment_id: " . $e->getMessage();
        }
    }

    // Check if we're done
    $total_attachments = wp_count_posts('attachment')->inherit;
    $is_complete = ($offset + $batch_size) >= $total_attachments;
    ...
    $response_data = [
        ...
        'processed' => min($offset + $batch_size, $total_attachments)
    ];
...
?>
    <div class="wrap" style="background: var(--clr-page-bg); padding: 20px; min-height: 100vh;">
        <div class="fcc-header-section" style="width: 1280px; margin: 0 auto;">
            <h1 class="text-2xl font-bold mb-4">Media Inventory Forge (1.0)</h1><br>
